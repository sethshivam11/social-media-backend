...
const createStory = asyncHandler(
    async (req: Request, res: Response) => {
        ...
        const saveToCloudinary = await Promise.all(files.map(async (file: File) => uploadToCloudinary(file.path, true)))
        ...
        return res.status(201).json(
            new ApiResponse(201, story, "Story uploaded")
        )
    }
)
...
const getStories = asyncHandler(
    async (req: Request, res: Response) => {
        ...
        return res.status(200).json(
            new ApiResponse(200, stories, "Stories found")
        )
    }
)

const getUserStory = asyncHandler(
    async (req: Request, res: Response) => {
        if (!req.user) {
            throw new ApiError(401, "User not verified")
        }
...
const deleteStory = asyncHandler(
    async (req: Request, res: Response) => {
        ...
        await Promise.all(mediaFiles.map(async (link: string) => deleteFromCloudinary(link)))
...
const seenStory = asyncHandler(
    async (req: Request, res: Response) => {
        ...
        return res.status(200).json(
            new ApiResponse(200, story, "Story seen")
        )
    }
)

const likeStory = asyncHandler(
    async (req: Request, res: Response) => {
        if (!req.user) {
            throw new ApiError(401, "User not verified")
        }
        ...
        return res.status(200).json(
            new ApiResponse(200, story, "Story liked")
        )
    }
)

const unlikeStory = asyncHandler(
    async (req: Request, res: Response) => {
        if (!req.user) {
            throw new ApiError(401, "User not verified")
        }
        ...
        return res.status(200).json(
            new ApiResponse(200, {}, "Story unliked")
        )
    }
)
